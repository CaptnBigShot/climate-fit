...
const limitDegrees = (d) => {
  const x = d / 360
  let l = 360 * (x - Math.floor(x))
  if (l < 0) l += 360
  return l
}
const poly3 = (a, b, c, d, x) => x * (x * (x * a + b) + c) + d
const periodic = (terms, jme) =>
  terms.reduce((s, row, i) => s + row.reduce((t, [a, b, c]) => t + a * Math.cos(c * jme + b), 0) * jme ** i, 0) / 1e8

function spa(jd) {
  const jde = jd + 60 / 86400,
    jce = (jde - 2451545) / 36525,
    jme = jce / 10
  const l = limitDegrees(periodic(L_TERMS, jme) / DEG),
    b = periodic(B_TERMS, jme) / DEG,
    r = periodic(R_TERMS, jme)
  const theta0 = l + 180,
    theta = theta0 >= 360 ? theta0 - 360 : theta0,
    beta = -b
  const x = [
  ...
  for (let i = 0; i < Y_TERMS.length; i++) {
    const y = Y_TERMS[i],
      arg = (x[0] * y[0] + x[1] * y[1] + x[2] * y[2] + x[3] * y[3] + x[4] * y[4]) * DEG
    psi += (jce * PE_TERMS[i][1] + PE_TERMS[i][0]) * Math.sin(arg)
    eps += (jce * PE_TERMS[i][3] + PE_TERMS[i][2]) * Math.cos(arg)
  }
  const delPsi = psi / 36e6,
    delEps = eps / 36e6
  const u = jme / 10
  const eps0 = [2.45, 5.79, 27.87, 7.12, -39.05, -249.67, -51.38, 1999.25, -1.55, -4680.93, 84381.448].reduce(
    (a, c, i) => (i === 0 ? c : u * a + c),
    0,
  )
  ...
  const alpha = limitDegrees(
    Math.atan2(
      Math.sin(lamda * DEG) * Math.cos(epsilon * DEG) - Math.tan(beta * DEG) * Math.sin(epsilon * DEG),
      Math.cos(lamda * DEG),
    ) / DEG,
  )
  const delta =
    Math.asin(
      Math.sin(beta * DEG) * Math.cos(epsilon * DEG) +
        Math.cos(beta * DEG) * Math.sin(epsilon * DEG) * Math.sin(lamda * DEG),
    ) / DEG
  const m = limitDegrees(
    jme * (jme * (jme * (jme * (jme * (-1 / 2e6) - 1 / 15300) + 1 / 49931) + 0.03032028) + 360007.6982779) +
      280.4664567,
  )
  let eot = 4 * (m - 0.0057183 - alpha + delPsi * Math.cos(epsilon * DEG))
  if (eot < -20) eot += 1440
  else if (eot > 20) eot -= 1440
  return { delta, eot }
}

// ---------- lookup: every 20 days from 1950, Hermite ring ----------

const REF_START = Date.UTC(1950, 0, 1) / 1000,
  REF_DT = 86400 * 20,
  REF_SPAN = Date.UTC(2050, 0, 1) / 1000 - REF_START
let table = null
function lookup() {
  if (!table) {
    const n = Math.ceil(REF_SPAN / REF_DT),
      decl = new Float32Array(n),
      eot = new Float32Array(n)
    for (let i = 0; i < n; i++) {
...
function sunAt(sec) {
  ...
  const i = Math.floor(x / REF_DT),
    t = f(f(x % REF_DT) / REF_DT)
...
export function sunshineDuration(direct, lat, lon, startSec) {
  const dt = 3600,
    out = new Float32Array(direct.length)
  const t0 = rad(f(90 - lat))
  const sin0 = f(Math.sin(t0)),
    cos0 = f(Math.cos(t0))
  for (let i = 0; i < direct.length; i++) {
    ...
    const sec = startSec + i * dt,
      { decl, eqtime } = sunAt(sec)
