...
async function fetchEpa(cities, dir, endYear) {
  ...
  const t0 = Date.UTC(EPA_START_YEAR, 0, 1), n = (Date.UTC(endYear, 11, 31) - t0) / DAY + 1
  ...
  const best = cities.map(() => Object.fromEntries(POLLUTANTS.map((p) => [p.key, {
    aqi: new Int16Array(n).fill(-1), km: new Float32Array(n).fill(Infinity), site: new Array(n),
  ...
  for (let year = EPA_START_YEAR; year <= endYear; year++) {
    for (const p of POLLUTANTS) {
      ...
      if (!zip) { console.log(`  airdata: EPA has not published ${p.label} for ${year}`); continue }
      for await (const [id, row] of readCsv(zip, (id) => near.has(id))) {
        ...
        for (const { ci, km } of near.get(id)) {
          ...
          if (b.readings) {
            ...
            b.km[d] = km; b.aqi[d] = aqi; b.site[d] = id
          }
        }
      }
    }
  ...
  for (const b of best.flatMap((c) => Object.values(c))) {
    b.readings?.forEach((day, d) => { [b.site[d], b.aqi[d]] = pickHighest(day) })
  }
  ...
  for (const [ci, city] of cities.entries()) {
    ...
    for (const p of measured) {
      ...
      first = Math.min(first, a.findIndex((v) => v >= 0))
      last = Math.max(last, a.findLastIndex((v) => v >= 0))
    }
    const aqi = {}, used = {}
    for (const p of measured) {
      const b = best[ci][p.key]
      ...
      used[p.key] = [...days].sort((x, y) => y[1] - x[1]).map(([id, count]) => ({
        id, name: siteById.get(id).name, km: Number(distanceKm(city, siteById.get(id)).toFixed(1)), days: count,
      }))
    }
...
async function readSites(dir) {
  ...
  for await (const [id, r] of readCsv(zip, () => true)) {
    const lat = Number(r.Latitude), lon = Number(r.Longitude), closed = r['Site Closed Date']
...
async function download(dir, name) {
  ...
  const res = await fetch(`${AIRDATA}/${name}.zip`, { headers: cached ? { 'If-Modified-Since': cached.mtime.toUTCString() } : {} })
...
async function* readCsv(zip, keep) {
  ...
  const exit = new Promise((resolve) => { unzip.on('error', resolve); unzip.on('close', resolve) })
  let head = null
  for await (const line of createInterface({ input: unzip.stdout, crlfDelay: Infinity })) {
    if (!head) { head = parseCsvLine(line); continue }
    const id = line.split(',', 3).map((f) => f.replaceAll('"', '')).join('-')
    if (!keep(id)) continue
