...
export function budget(sc: Scored): Budget {
  const counts = [0, 0, 0]
  let hard = 0, cur = 0, gap = 0, unb = 0, bestStreak = 0, worstGap = 0, unbStreak = 0
  const perYear: number[] = []
  const months = Array.from({ length: 12 }, () => ({ c: 0, t: 0, u: 0 }))
  const reasons = new Map<number, number>(), compro = new Map<number, number>()
  for (let y = 0; y < sc.years; y++) {
    let yc = 0
    for (let d = 0; d < 365; d++) {
      const i = y * 365 + d, b = sc.band[i], m = months[doyMonth(d)]
      counts[b]++
      if (sc.hard[i]) hard++
      if (b === BAND.comf) { yc++; m.c++; cur++; gap = 0; unb = 0; bestStreak = Math.max(bestStreak, cur) }
      else {
        cur = 0; gap++; worstGap = Math.max(worstGap, gap)
        if (b === BAND.tol) { m.t++; unb = 0 } else { m.u++; unb++; unbStreak = Math.max(unbStreak, unb) }
        const why = sc.why[i]
        if (why) {
  ...
  return {
    counts: [counts[0] * k, counts[1] * k, counts[2] * k], hard: hard * k,
    perYear, trend: ols(perYear), months,
    ...
    bestStreak, unbStreak, worstGap, worstMonth, bestMonth,
  }
}

// ---------- Terrain ----------

export interface TerrainChoice { id: string; driveMin: number; rideDays: number }

/** Spec 6.12: within the drive-time stop, the reference with the most snow days over the window. */
export function resolveTerrain(cands: { id: string; driveMin: number; series: TerrainSeries }[], w: Window): TerrainChoice | null {
  let best: TerrainChoice | null = null
  for (const c of cands) {
    const off = (w.from - c.series.startYear) * 365, N = windowYears(w) * 365
    let n = 0
    for (let i = 0; i < N; i++) if (c.series.depth[off + i] >= RIDE_DEPTH_IN) n++
    const rideDays = n / windowYears(w)
    if (!best || rideDays > best.rideDays || (rideDays === best.rideDays && c.driveMin < best.driveMin)) best = { id: c.id, driveMin: c.driveMin, rideDays }
  }
  return best
}
...
const dayInputs = (s: CitySeries, j: number, depth: Float32Array | null, dj: number): DayInputs => ({
  hi: s.high[j], lo: s.low[j], dew: s.dew[j], precip: s.precip[j], wind: s.wind[j], depth: depth ? depth[dj] : NaN,
})
...
export function activities(s: CitySeries, w: Window, acts: ActivityId[], terrain: TerrainSeries | null): ActivityAgg {
  ...
  const per = Object.fromEntries(ACTIVITIES.map((a) => [a.id, { days: 0, lost: [0, 0, 0, 0, 0, 0] }])) as ActivityAgg['per']
  const lost = [0, 0, 0, 0, 0, 0], months = new Array(12).fill(0)
  const walkOk = new Float32Array(365)
  let outAny = 0
  for (let y = 0; y < years; y++) {
    for (let d = 0; d < 365; d++) {
      const i = y * 365 + d, inp = dayInputs(s, off + i, terrain?.depth ?? null, tOff + i)
      let any = false, firstLoss: LossReason = 0
      ...
      if (any) { outAny++; months[doyMonth(d)]++ } else if (enabled.length) lost[firstLoss]++
    }
  }
  const k = 1 / years
  for (const a of ACTIVITIES) { per[a.id].days *= k; per[a.id].lost = per[a.id].lost.map((v) => v * k) }
  return { per, outAny: outAny * k, lost: lost.map((v) => v * k), months: months.map((v) => v * k), season: outdoorSeason(walkOk, years) }
}
...
function outdoorSeason(walkOk: Float32Array, years: number): ActivityAgg['season'] {
  ...
  for (let k = 0; k < 730; k++) {
    if (ok[k % 365]) { run++; if (run > best && run <= 365) { best = run; bestEnd = k } } else run = 0
  }
  if (!best) return null
...
export interface MonthRow {
  m: number; hi: number; lo: number; rhi: number; rlo: number; swing: number; dew: number; cloud: number
  precip: number; snow: number; sun: number; over: number | null; comf: number | null; out: number
  split: { c: number; t: number; u: number } | null
}

export function monthly(s: CitySeries, w: Window, hardMax: number | null, b: Budget | null, act: ActivityAgg): MonthRow[] {
  const years = windowYears(w), off = (w.from - FIRST_YEAR) * 365
  return MD.map((len, m) => {
    let hi = 0, lo = 0, dew = 0, cloud = 0, precip = 0, snow = 0, sun = 0, n = 0, over = 0, rhi = -Infinity, rlo = Infinity
    for (let y = 0; y < years; y++) {
      for (let d = MONTH_START[m]; d < MONTH_START[m] + len; d++) {
        const j = off + y * 365 + d
        hi += s.high[j]; lo += s.low[j]; dew += s.dew[j]; cloud += s.cloud[j]
        precip += s.precip[j]; snow += s.snow[j]; sun += s.sun[j]; n++
        rhi = Math.max(rhi, s.high[j]); rlo = Math.min(rlo, s.low[j])
        if (hardMax !== null && s.high[j] > hardMax) over++
      }
    }
    return {
      m, hi: hi / n, lo: lo / n, rhi, rlo, swing: (hi - lo) / n, dew: dew / n, cloud: cloud / n,
      precip: precip / years, snow: snow / years, sun: sun / n,
      over: hardMax === null ? null : over / years,
      comf: b ? b.months[m].c / years : null, out: act.months[m], split: b ? b.months[m] : null,
    }
  })
}
...
export interface CounterAgg { perYear: number[]; avg: number; fit: Fit }
...
export interface Facts {
  swing: number
  freezeThaw: number
  clear: number; partly: number; overcast: number
  grayStreak: number
  warmNights: number
  snowDays: number
  sunHours: number
...
export function daylightHours(lat: number, doy: number): number {
  ...
  const phi = (lat * Math.PI) / 180, del = (decl * Math.PI) / 180
...
export function facts(s: CitySeries, w: Window, lat: number): Facts {
  const years = windowYears(w), off = (w.from - FIRST_YEAR) * 365, N = years * 365
  let swing = 0, ft = 0, clear = 0, partly = 0, overcast = 0, gray = 0, grayBest = 0, warmN = 0, snowD = 0, sun = 0
  const monthT = new Float64Array(12)
  for (let i = 0; i < N; i++) {
    const j = off + i, hi = s.high[j], lo = s.low[j], cl = s.cloud[j]
    swing += hi - lo
    if (lo <= 32 && hi > 32) ft++
    if (cl <= 20) clear++; else if (cl >= 80) overcast++; else partly++
    if (cl >= 80) { gray++; grayBest = Math.max(grayBest, gray) } else gray = 0
    if (lo > 60) warmN++
  ...
  const warmest = mMeans.indexOf(Math.max(...mMeans)), coldest = mMeans.indexOf(Math.min(...mMeans))
  ...
  const last: number[] = [], first: number[] = []
  let freezeYears = 0
  for (let y = 0; y < years; y++) {
    let l = -1, f = -1
    for (let k = 1; k <= 182; k++) {
      const back = pivot - k, yb = y + Math.floor(back / 365)
      if (l < 0 && yb >= 0 && s.low[off + yb * 365 + ((back % 365) + 365) % 365] <= 32) l = (back + 365) % 365
      const fwd = pivot + k, yf = y + Math.floor(fwd / 365)
      if (f < 0 && yf < years && s.low[off + yf * 365 + (fwd % 365)] <= 32) f = fwd % 365
    }
    if (l >= 0 && f >= 0) { last.push(l); first.push(f); freezeYears++ }
  }
  ...
  let dMin = 24, dMax = 0
  for (let d = 0; d < 365; d += 1) { const h = daylightHours(lat, d); dMin = Math.min(dMin, h); dMax = Math.max(dMax, h) }
  return {
    swing: swing / N, freezeThaw: ft / years, clear: clear / years, partly: partly / years, overcast: overcast / years,
    grayStreak: grayBest, warmNights: warmN / years, snowDays: snowD / years, sunHours: sun / years,
    lastFreeze, firstFreeze,
    frostFree: lastFreeze !== null && firstFreeze !== null ? (firstFreeze - lastFreeze + 365) % 365 : null,
    freezeYears, warmestMonth: warmest, coldestMonth: coldest, daylight: [dMin, dMax],
  }
}
...
export function terrainCover(t: TerrainSeries, w: Window, minDepth: number): number {
  const off = (w.from - t.startYear) * 365, years = windowYears(w)
  let n = 0
