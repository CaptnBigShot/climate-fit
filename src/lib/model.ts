...
export function terrainAt(
  ...
  return choice
    ? { ...choice, name: TERRAIN[choice.id].name, elevFt: TERRAIN[choice.id].elevFt, series: terrains[choice.id] }
    : null
}
...
export function buildModel(
  ...
  if (b && p.temp) {
    ...
    if (t.hardMax !== null) {
      reclaim.push({
        text: `Raising your ${fmtT(t.hardMax)}${tu} ceiling to ${fmtT(t.hardMax + RECLAIM_STEP)}${tu}`,
        days: gain({ temp: { ...t, hardMax: t.hardMax + RECLAIM_STEP } }),
      })
    }
    if (t.hardMin !== null) {
      reclaim.push({
        text: `Lowering your ${fmtT(t.hardMin)}${tu} floor to ${fmtT(t.hardMin - RECLAIM_STEP)}${tu}`,
