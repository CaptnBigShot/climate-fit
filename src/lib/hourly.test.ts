...
describe('loadHourly', () => {
  it('fetches a city with no hourly file live from Open-Meteo', async () => {
    ...
    for (let t = Date.UTC(2016, 0, 1); time.length < days * 24; t += 3600_000) {
      ...
      if (!iso.startsWith('2016-02-29') && !iso.startsWith('2020-02-29') && !iso.startsWith('2024-02-29'))
        time.push(iso)
    }
    const urls: string[] = []
    vi.stubGlobal('fetch', async (url: string) => {
      ...
      if (String(url).endsWith('manifest.json'))
        return Response.json({ refYears: 10, cities: {}, terrain: [], hourly: [withFile.id] })
    ...
    expect(
      urls.some((u) => u.startsWith('https://archive-api.open-meteo.com/') && u.includes(`latitude=${without.lat}`)),
    ).toBe(true)
  })
})
