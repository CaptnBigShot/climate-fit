...
export interface TempBand { hardMin: number | null; idealMin: number; idealMax: number; hardMax: number | null }
export interface Window { from: number; to: number }
export interface CustomCounter { m1: CounterMetric; op1: CounterOp; v1: number; m2: CounterMetric | null; op2: CounterOp; v2: number }
...
export interface Preset { name: string; apply: Partial<Prefs> }
...
export const PRESETS: Preset[] = [
  { name: 'Cold & Dry',
    apply: { temp: { hardMin: null, idealMin: 20, idealMax: 45, hardMax: 60 }, seasonal: false, dewMax: 35, cloud: 'any', dry: true } },
  { name: 'Cool & Overcast',
    apply: { temp: { hardMin: 25, idealMin: 45, idealMax: 62, hardMax: 72 }, seasonal: false, dewMax: 50, cloud: 'overcast', dry: false } },
  { name: 'Four True Seasons',
    apply: { temp: { hardMin: -10, idealMin: 68, idealMax: 84, hardMax: 95 }, seasonal: true, cold: { idealMin: 20, idealMax: 40 }, dewMax: 62, cloud: 'any', dry: false } },
  { name: 'Mild Year-Round',
    apply: { temp: { hardMin: 40, idealMin: 60, idealMax: 75, hardMax: 88 }, seasonal: false, dewMax: 58, cloud: 'any', dry: false } },
  { name: 'Snow Seeker',
    apply: { temp: { hardMin: null, idealMin: 55, idealMax: 75, hardMax: 85 }, seasonal: true, cold: { idealMin: 15, idealMax: 32 }, dewMax: 55, cloud: 'any', dry: false } },
  { name: 'Warm & Dry',
    apply: { temp: { hardMin: 55, idealMin: 78, idealMax: 92, hardMax: 105 }, seasonal: false, dewMax: 50, cloud: 'clear', dry: true } },
  { name: 'Warm & Humid',
    apply: { temp: { hardMin: 60, idealMin: 80, idealMax: 90, hardMax: 98 }, seasonal: false, dewMax: 72, cloud: 'any', dry: false } },
]
...
export const EXAMPLE_STATE: Partial<Prefs> = {
  ...
  seasonal: true, cold: { idealMin: 25, idealMax: 48 },
  dewMax: 45, cloud: 'overcast', drive: 2, acts: ['walk', 'ride'],
}
...
export function encodePrefs(p: Prefs, q: URLSearchParams) {
  ...
  if (p.temp) q.set('b', [openNum(p.temp.hardMin, true), p.temp.idealMin, p.temp.idealMax, openNum(p.temp.hardMax, false)].join(','))
  ...
  const wt = (Object.keys(p.weights) as WeightedVar[]).filter((k) => p.weights[k] !== 'normal').map((k) => `${k}.${WEIGHT_CODE[p.weights[k]]}`)
  ...
  if (p.counters.length) q.set('ctr', p.counters.map((c) => [c.m1, c.op1, c.v1, c.m2 ?? '', c.op2, c.v2].join('.')).join('~'))
}
...
export function decodePrefs(q: URLSearchParams): Prefs {
  ...
  if (b && b.length === 4) {
    const iMin = num(b[1]), iMax = num(b[2])
  ...
  if (cb && cb.length === 2 && cb[0] !== null && cb[1] !== null) { p.seasonal = true; p.cold = { idealMin: cb[0], idealMax: cb[1] } }
  p.dewMax = num(q.get('dp'))
  const s = q.get('s'); if (s === 'lenient' || s === 'standard' || s === 'strict') p.strict = s
  const snow = Number(q.get('snow')); if (snow === 1 || snow === 2 || snow === 3) p.drive = snow
  const w = q.get('w')
  if (w) {
    const years = /^(\d+)y$/.exec(w), range = /^(\d{4})-(\d{4})$/.exec(w)
    if (years) p.window = lookbackWindow(Math.min(Math.max(1, +years[1]), LAST_YEAR - FIRST_YEAR + 1))
    else if (range) {
      const from = Math.max(FIRST_YEAR, +range[1]), to = Math.min(LAST_YEAR, +range[2])
      if (from <= to) p.window = { from, to }
    }
  }
  if (q.get('sun') === 'sun') p.sun = 'sun'
  p.metric = q.get('u') === 'metric'
  const basis = q.get('basis'); if (basis === 'low' || basis === 'apparent' || basis === 'both') p.basis = basis
  const cloud = q.get('cloud'); if (cloud === 'clear' || cloud === 'overcast') p.cloud = cloud
  ...
  for (const c of q.get('ctr')?.split('~') ?? []) {
    ...
    p.counters.push({ m1: m1 as CounterMetric, op1: op1 as CounterOp, v1: +v1,
      m2: m2 && isM(m2) ? (m2 as CounterMetric) : null, op2: isOp(op2) ? (op2 as CounterOp) : 'ge', v2: Number.isFinite(+v2) ? +v2 : 0 })
  }
  return p
}
...
export function presetNote(pr: Preset, t: (f: number) => string, tu: string): string {
  const a = pr.apply, parts: string[] = []
  if (a.temp) {
    parts.push(a.seasonal && a.cold
