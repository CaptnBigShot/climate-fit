...
import { score, causeLabel, dayCause } from './scoring'
...
const header = (city: CityMeta, p: Prefs, u: Units) =>
  `# Climate Fit · ${city.name}, ${city.code} · ${u.metric ? '°C/mm/km/h' : '°F/in/mph'} · window ${windowLabel(p.window)}\n`
...
export function downloadMonthlyCsv(city: CityMeta, rows: MonthRow[], p: Prefs, u: Units) {
  ...
  const lines = rows.map((r) =>
    [
      ...
      t(r.rhi),
      t(r.rlo),
      (u.metric ? (r.swing * 5) / 9 : r.swing).toFixed(1),
      t(r.dew),
      r.cloud.toFixed(0),
      len(r.precip),
      len(r.snow),
...
export function downloadDailyCsv(city: CityMeta, s: CitySeries, p: Prefs, u: Units) {
  ...
  for (let i = 0; i < N; i++) {
    const j = off + i,
      y = p.window.from + Math.floor(i / 365),
      d = i % 365,
      m = doyMonth(d)
    ...
    lines.push(
      [
        ...
        sc ? causeLabel(dayCause(sc, i)) : '',
      ].join(','),
    )
  }
  download(
    `climate-fit-${city.id}-daily-${p.window.from}-${p.window.to}.csv`,
    header(city, p, u) + head.join(',') + '\n' + lines.join('\n') + '\n',
  )
}
