...
export const Compare = memo(function Compare({
  ...
  const present = useMemo(
    () => ids.map((id) => rows.find((r) => r.city.id === id)).filter((r): r is CityRow => !!r),
    [ids, rows],
  )
    ...
    () =>
      [...present].sort(
        scored
          ...
          : (a, b) => b.out.act.per.walk.days - a.out.act.per.walk.days,
      ),
    [present, scored],
  )
  ...
  const comf = useMemo(
    () => new Map(rows.flatMap((r) => (r.fit ? [[r.city.id, Math.round(r.fit.b.counts[0])] as const] : []))),
    [rows],
  )
  ...
  return (
    <main className="page">
      <div className="view-head">
        ...
        <span className="sub">
          {ids.length} of {MAX_COMPARE} cities · one session state · {win}
        </span>
        <div className="chips">
          {ids.map((id) => {
            const c = cityById(id)!,
              r = present.find((x) => x.city.id === id)
            return (
              <button
                key={id}
                className="cmp-chip"
                onClick={() => setCmp(toggleCompare(ids, id))}
              ...
              >
                {c.name}
                {r?.fit && <span className="n">{Math.round(r.fit.b.counts[0])}</span>}
        ...
        <span className="sub view-end">
          {ids.length === 1
            ? 'add at least one more city to compare'
            : scored
              ? 'ordered by comfortable days · no winner picked'
              : 'not scored · ordered by walk-viable days'}
        </span>
      </div>
      ...
      {!ids.length ? (
        <div className="section" style={{ borderBottom: 'none' }}>
          <div className="empty">
            ...
            <div className="prose" style={{ fontSize: 12.5 }}>
              Add two to four cities — here, or with “+ Add to compare” on any city page. Every city is scored on the
              same settings in the control bar, and the set lives in the URL, so sending the link sends the comparison.
            </div>
        ...
        <>
          ...
          <div className="cmp-cols">
            {ordered.map((r) => (
              <div className="cmp-col" key={r.city.id}>
                {r.fit ? <WhyShort r={r} p={p} u={u} /> : <Record r={r} p={p} u={u} />}
              </div>
            ))}
          </div>
...
function AddCity({
  ...
  return (
    <div className="add-city" ref={ref}>
      <button className="btn primary" disabled={full} aria-expanded={open} onClick={() => setOpen(!open)}>
        {full ? 'Set full' : '+ Add city'}
      </button>
      {open && (
...
function BudgetRow({
  ...
  const b = r.fit?.b,
    ...
    t = r.out.terrain
  const snow = t
    ? `${t.name} · ${t.driveMin} min`
    : hasTerrainWithin(r.city, p.drive)
      ? 'loading terrain…'
      : r.city.terrain.length
        ? `none within ${p.drive} hr`
        : 'no terrain on record'
  return (
    <div className="cmp-row">
      <div className="who">
        <button className="city-link" onClick={() => openCity(r.city.id)} data-tip={`Open ${r.city.name}'s dashboard.`}>
          {r.city.id === current && <span className="dot">●</span>}
          {r.city.name}, {r.city.code}
        </button>
        <div className="meta">
          {u.elev(r.s.demElevM * FT)} · pop {r.city.pop.toLocaleString()} · solar {solar ?? '—'}/100
        </div>
        <div className="meta" style={{ color: 'var(--dim)' }}>
          SNOW · {snow}
        </div>
      </div>
      ...
      <div className="stats">
        ...
        <Mini
          ...
          color={CO.act}
          v={act.outAny}
          tip="Days a year on which at least one enabled activity is possible. Scored separately from comfort."
        />
        <Mini
          label="RIDE"
          color={CO.actSnow}
          v={act.per.ride.days}
          tip={`Snow-sport days a year: at least ${u.depth(18)} on the ground at the terrain resolved within ${p.drive} hr.`}
        />
      </div>
    </div>
  )
    ...
    <svg width={width} height={h + 20} style={{ display: 'block', shapeRendering: 'crispEdges' }}>
      ...
      {[0, 91, 182, 273, 365].map((d) => (
        <text
          ...
          x={x(d)}
          y={h + 15}
          textAnchor={d === 0 ? 'start' : d === 365 ? 'end' : 'middle'}
          fill="#5f6672"
...
function Small({ k, v, tip }: { k: string; v: string; tip?: string }) {
  return (
    <div>
      <Cap tip={tip}>{k}</Cap>
      <div className="mono" style={{ font: '500 14px/1.3 var(--mono)', marginTop: 3 }}>
        {v}
      </div>
    </div>
  )
    ...
    <>
        ...
        {b.reasons.slice(0, 3).map((x) => (
          <div className="row" key={x.label}>
            ...
            <span className="track">
              <span style={{ width: `${Math.max(1, x.pct)}%` }} />
            </span>
      ...
      <div className="prose cmp-sep">
        ...
        {b.reasons[0] && (
          <>
            {' '}
            Most common shortfall overall: <b>{b.reasons[0].label}</b>, on {b.reasons[0].pct}% of its{' '}
            {Math.round(b.nonComf)} non-comfortable days a year.
          </>
        )}
      </div>
      <div className="cmp-sep" style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
        ...
        <Small
          k="WORST MONTH"
          v={MN[b.worstMonth].toUpperCase()}
          tip="The month with the smallest share of comfortable days for you."
        />
        <Small
          k="SNOW COVER"
          v={t ? `${Math.round(terrainCover(t.series, p.window, 6))} /yr` : '—'}
          tip={`Days a year with at least ${u.depth(6)} on the ground at the resolved reference terrain.`}
        />
      </div>
    </>
  )
...
function Calendars({
  ...
  const rowsBy = useMemo(
    () =>
      Object.fromEntries(
        ordered.map((r) => [r.city.id, windowRows(r.s, r.fit?.sc ?? null, p.window, r.out.terrain?.series ?? null)]),
      ),
    [ordered, p.window],
  )
  const title =
    mode === 'temp' ? 'TEMPERATURE CALENDARS' : mode === 'activity' ? 'ACTIVITY CALENDARS' : 'COMFORT CALENDARS'
  const options = scored
    ? [
        { v: 'comfort' as CalMode, label: 'COMFORT' },
        { v: 'activity' as CalMode, label: 'ACTIVITY' },
        { v: 'temp' as CalMode, label: 'TEMPERATURE' },
      ]
    : [
        { v: 'temp' as CalMode, label: 'TEMPERATURE' },
        { v: 'activity' as CalMode, label: 'ACTIVITY' },
  ...
  return (
    <div className="section">
      <div className="section-head">
        <Head tip="Stacked on one day-of-year axis, so a seasonal offset between cities reads as a horizontal shift rather than something you have to hold in your head. Hover a cell for that day's conditions.">
          {title} · SHARED AXIS
        </Head>
        <span className="sub">
          one cell = one observed day · {windowLabel(p.window)}
          {mode === 'temp' ? ' · daily high · one scale for every city' : ''}
        </span>
        <div className="right">
          {mode === 'temp' && <TempLegend u={u} />}
          <Seg
            small
            label="Calendar colouring"
            value={mode}
            onChange={scored ? setScoredMode : setUnsetMode}
            options={options}
          />
        </div>
      </div>
        ...
        return (
          <div key={r.city.id} style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 5 }}>
              ...
              <span className="sub">
                {b
                  ? `${Math.round(b.counts[0])} comf · ${Math.round(b.counts[1])} tol · ${Math.round(b.counts[2])} unb per year`
                  : `${Math.round(r.out.act.per.walk.days)} walk-viable days per year`}
              </span>
            </div>
      ...
      {mode === 'activity' && (
        <div className="legend">
          <span className="item">
            <span className="sw" style={{ background: CO.act }} />
            AN ENABLED ACTIVITY IS POSSIBLE
          </span>
          <span className="item">
            <span className="sw" style={{ background: CO.actSnow }} />
            SNOW-SPORT DAY
          </span>
          <span className="item">
            <span className="sw" style={{ background: CO.actNone }} />
            NONE POSSIBLE
          </span>
        </div>
      )}
      {mode === 'temp' && (
        <div className="legend">
          <span className="prose" style={{ fontSize: 11, color: 'var(--dim)' }}>
            Observed temperature, not fit: the ramp is shared by every city and deliberately unlike the comfort colours.
          </span>
        </div>
      )}
    </div>
...
function Pivot({ ordered, p, u, scored }: { ordered: CityRow[]; p: Prefs; u: Units; scored: boolean }) {
  ...
  const months = useMemo(
    () => ordered.map((r) => monthly(r.s, p.window, hardMax, r.fit?.b ?? null, r.out.act)),
    [ordered, p.window, hardMax],
  )
    ...
    def.kind === 'temp'
      ...
      : def.kind === 'pct'
        ...
        : def.kind === 'len'
          ...
          : v.toFixed(1)
  const tol =
    def.kind === 'temp' ? `${spread(def.tol)}${u.tu}` : def.kind === 'days' ? `${def.tol} days` : spread(def.tol)
  const options = (Object.keys(PIVOT) as PivotMetric[])
    .filter((k) => scored || !PIVOT[k].needsFit)
    .map((k) => ({ v: k, label: PIVOT[k].label.toUpperCase() }))
  return (
    <div className="section" style={{ borderBottom: 'none' }}>
      <div className="section-head">
        <Head tip="Monthly rollups pivoted city by city. Differences-only hides the months where the cities sit within a tolerance of each other, leaving what actually separates them. Counts are days per year in that month.">
          MONTHLY, PIVOTED
        </Head>
        <span className="sub">
          {diff
            ? `${rows.length - shown.length} months hidden · spread under ${tol}`
            : `all 12 months · tolerance ${tol}`}
        </span>
        <div className="right">
          <Seg small label="Pivot measure" value={metric} onChange={setMetric} options={options} />
          <div className="seg sm">
            <button aria-pressed={diff} onClick={() => setDiff(!diff)}>
              DIFFERENCES ONLY
            </button>
          </div>
        </div>
        ...
        <table className="plain">
          <thead>
            <tr>
              ...
              {ordered.map((r) => (
                <th key={r.city.id}>{r.city.name.toUpperCase()}</th>
              ))}
          ...
          <tbody>
            {shown.map((row) => (
              <tr key={row.m}>
                ...
                {row.vals.map((v, i) => {
                  const hi = multi && v === row.max,
                    lo = multi && v === row.min
                  return (
                    <td
                      key={i}
                      style={{ color: hi ? 'var(--ink)' : lo ? '#767d8a' : 'var(--mid)', fontWeight: hi ? 500 : 400 }}
                    >
            ...
            {!shown.length && (
              <tr>
                <td className="l" colSpan={ordered.length + 2} style={{ color: 'var(--dim)', padding: '10px 8px' }}>
                  No month differs by more than {tol} — on this measure the cities are alike.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="prose" style={{ marginTop: 12, fontSize: 11, color: 'var(--dim)' }}>
        Brightest figure in each row is the highest in the set, dimmest the lowest. No city is named a winner — more
        snow against fewer heat days is not a comparison the app is entitled to settle.
      </div>
    </div>
  )
}
