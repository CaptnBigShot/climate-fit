...
export function Header({ city, m, p, u, elevFt, solarIdx, inCompare, compareCount, toggleCompare, exportCsv }: {
  ...
  return (
    <div className="header">
      <div>
        ...
        <div className="city-region">{city.region} · {Math.abs(city.lat).toFixed(2)}°{city.lat >= 0 ? 'N' : 'S'} {Math.abs(city.lon).toFixed(2)}°{city.lon >= 0 ? 'E' : 'W'}</div>
      </div>
      <div style={{ display: 'flex', gap: 26, paddingBottom: 3, flexWrap: 'wrap' }}>
        <div className="kv"><Cap tip="Elevation of the city-centre coordinate from a 90 m terrain model. Temperatures are downscaled to this height.">ELEVATION</Cap><div className="v">{u.elev(elevFt)}</div></div>
        <div className="kv"><Cap>POPULATION</Cap><div className="v">{city.pop.toLocaleString()}</div></div>
        <div className="kv">
          <Cap tip="The reference ski terrain used for snow-sport metrics: among the terrain inside your drive limit, the one with the most snow days over the window.">SNOW TERRAIN ≤{p.drive} HR</Cap>
          <div className="v">{m.terrain ? <>{m.terrain.name} <small>· {m.terrain.driveMin} min · {u.elev(m.terrain.elevFt)}</small></> : <small>none in reach</small>}</div>
        </div>
