...
export function CityList({
  ...
  const [active, setActive] = useState(() =>
    Math.max(
      0,
      matches.findIndex((c) => c.id === current),
    ),
  )
  ...
  useEffect(() => {
    const box = list.current,
      el = box?.children[at] as HTMLElement | undefined
    if (!box || !el) return
    if (!opened.current) {
      opened.current = true
  ...
  return (
    <div className="city-list">
      <input
        ...
        onKeyDown={onKey}
        placeholder={`Search ${CITIES.length - (exclude?.length ?? 0)} cities…`}
        spellCheck={false}
        autoComplete="off"
        role="combobox"
