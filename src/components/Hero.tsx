          ...
          <>
            ...
            <div style={{ borderTop: '1px solid var(--line)', paddingTop: 16 }}>
              <div style={{ marginBottom: 8 }}>
                <Head
                  small
                  tip="Across the tolerable days only, the condition that most often kept them short of comfortable — what you would actually be putting up with."
                >
                  COMPROMISE PROFILE
                </Head>
              </div>
            ...
            <div style={{ borderTop: '1px solid var(--line)', paddingTop: 16 }}>
              ...
              {m.reclaim.length ? (
                m.reclaim.map((r) => (
                  <div className="prose" key={r.text}>
                    {r.text} reclaims{' '}
                    <span className="mono" style={{ color: CO.comf }}>
                      {r.days}
                    </span>{' '}
                    days/yr here.
                  </div>
...
function Trend({ b, from, to }: { b: Budget; from: number; to: number }) {
  ...
  const first = b.perYear[0],
    last = b.perYear[b.perYear.length - 1]
  return (
    <div
      style={{
        display: 'flex',
    ...
    >
      ...
      <span className="prose" style={{ fontSize: 12 }}>
        Comfortable days: {first} in {from}, {last} in {to}. Fitted trend {b.trend.slope >= 0 ? '+' : '−'}
        {Math.abs(b.trend.slope).toFixed(1)} days/yr.{' '}
        <span className="mono" style={{ fontSize: 10.5, color: 'var(--faint)' }}>
          OLS · R² {b.trend.r2.toFixed(2)}
          {b.trend.r2 < 0.3 ? ' — weak fit, mostly year-to-year noise' : ''}
        </span>
      </span>
    </div>
  )
  ...
  const put = (x0: number, w: number, v: number) =>
    w > 26 ? (
      <text
        ...
        textAnchor="middle"
        fill="#0d0f12"
        style={{ font: "600 12px 'JetBrains Mono', monospace", pointerEvents: 'none' }}
      >
        {Math.round(v)}
      </text>
    ) : null
...
function YtdBlock({
  ...
  const head = (right: React.ReactNode) => (
    <div className="section-head" style={{ marginBottom: 10 }}>
      <Head
        small
        tip={`${YTD_YEAR} is still in progress, so it is never folded into a lookback window. Here it is compared only against the same calendar span — Jan 1 to the last observed day — in each year of your window.`}
      >
        {YTD_YEAR} SO FAR
      </Head>
      {right}
  ...
  const wrap = (children: React.ReactNode) => (
    <div style={{ marginTop: 18, paddingTop: 16, borderTop: '1px solid var(--line)' }}>{children}</div>
  )
    ...
    return wrap(
      <>
        {head(
          <>
            <span className="sub">
              jan 1 → {through ?? '—'} · {n} days
            </span>
            {badge}
          </>,
        )}
        <div className="prose">Too few observed days yet for a fair comparison.</div>
      </>,
    )
  }
  ...
  return wrap(
    <>
      {head(
        <>
          <span className="sub">
            jan 1 → {through} · {n} days · vs same dates, {windowLabel(p.window)} mean
          </span>
          {badge}
        </>,
      )}
      ...
      <div className="prose" style={{ marginTop: 10 }}>
        {yb && (
          <>
            Comfortable <b className="mono">{d(yb.ytd[0], yb.typical[0])}</b>, unbearable{' '}
            <b className="mono">{d(yb.ytd[2], yb.typical[2])}</b> days vs a typical year by {through}.{' '}
          </>
        )}
        Outdoor days <b className="mono">{out.ytd}</b> vs typical{' '}
        <span className="mono">{Math.round(out.typical)}</span> ({d(out.ytd, out.typical)}).
      </div>
    </>,
  )
}
