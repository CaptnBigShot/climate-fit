...
import { BAND, causeLabel, dayCause, dayTemp, type Scored } from '../lib/scoring'
...
const dayInputs = (row: CalRow, d: number) => {
  const j = row.j0 + d,
    t = row.terrain
  return {
    hi: row.series.high[j],
    lo: row.series.low[j],
    dew: row.series.dew[j],
    precip: row.series.precip[j],
    wind: row.series.wind[j],
...
function cellFill(row: CalRow, d: number, mode: CalMode, fill: CalFill, enabled: Activity[]) {
  ...
  if (mode === 'activity') {
    const a = actCode(row, d, enabled)
    return a === 2 ? CO.actSnow : a === 1 ? CO.act : CO.actNone
  }
...
function paint(ctx: CanvasRenderingContext2D, o: PaintSpec) {
  ...
  o.rows.forEach((row, k) => {
    ...
    if (row.observed < 365) {
      ...
      ctx.fillText(
        `THROUGH ${doyLabel(row.observed - 1).toUpperCase()} · NOT YET OBSERVED`,
        LEFT + row.observed * cw + 6,
        top + o.rowH / 2 + 0.5,
      )
    }
...
export function ComfortCalendar({
  s,
  sc,
  p,
  terrain,
  ...
  const allRows = useMemo<CalRow[]>(() => {
    ...
    if (ytd && ytd.raw.days > 0) {
      rows.push({
        ...
        sc: ytdSc,
        scBase: 0,
        j0: (YTD_YEAR - FIRST_YEAR) * 365,
        terrain: terrainId ? (ytd.tx[terrainId] ?? null) : null,
        observed: ytd.raw.days,
        ytd: true,
      })
    }
    return rows
  ...
  const notYet = ytdRow && (
    <span className="item">
      <span className="sw" style={{ border: '1px solid #3c4149' }} />
      NOT YET OBSERVED
    </span>
  )
  ...
  return (
    <div className="section">
      <div className="section-head">
        {mode === 'temp' ? (
          <>
            <Head tip="Every observed day coloured by its daily high on one fixed temperature scale, the same for every city. This is the record as measured: no preference is involved, so no colour here means good or bad.">
              {cityName} AS MEASURED
            </Head>
            <span className="sub">
              daily high · one cell = one observed day · temperature ramp, not a comfort ramp · {win}
            </span>
          </>
        ) : (
          <>
            <Head
              tip={`One cell per observed day, no averaging — rows are years, columns are days of the year. This is where you see whether the good days cluster or scatter, and whether the pattern is drifting. The ${YTD_YEAR} row below the line is the year so far: it is shown, never averaged into the window. Click a year label to isolate it.`}
            >
              {mode === 'activity' ? 'ACTIVITY CALENDAR' : 'COMFORT CALENDAR'}
            </Head>
        ...
        {iso !== null && (
          ...
          >
            {allRows[iso].year} ISOLATED · CLEAR ✕
          </button>
        )}
        <div className="right">
      ...
      {mode === 'comfort' && (
        <BandLegend>
          ...
          <span className="cap" style={{ marginLeft: 'auto' }}>
            HOVER A CELL FOR FULL CONDITIONS · CLICK A YEAR TO ISOLATE
          </span>
        </BandLegend>
      )}
      {mode === 'activity' && (
        <div className="legend">
          ...
          {notYet}
          <span className="cap" style={{ marginLeft: 'auto' }}>
            ACTIVITY ENCODING · SEPARATE FROM COMFORT
          </span>
        </div>
      )}
      {mode === 'temp' && (
        <div className="legend">
          <span className="prose" style={{ fontSize: 11, color: 'var(--dim)' }}>
            No cell here means good or bad. Banding applies only to the derived comfort score, never to observed data,
            and this ramp is deliberately unlike the comfort one so the two can never be confused.
          </span>
          {notYet}
        </div>
      )}
      {hover && rows[hover.r] && (
        <DayTip row={rows[hover.r]} d={hover.d} p={p} u={u} x={hover.x} y={hover.y} mode={mode} />
      )}
    </div>
  )
}
...
export function CalendarStrip({
  ...
  useEffect(() => {
    ...
    paint(ctx, {
      ...
      hover,
      label: (row, k) =>
        rowH >= 8 || k === 0 || k === rows.length - 1 || row.year % 5 === 0 ? { color: '#5f6672' } : null,
    })
  ...
  const onMove = (e: React.MouseEvent) => {
    ...
    if (x < LEFT || y < pad || r < 0 || r >= rows.length) {
      setHover(null)
      return
    }
...
export function TempLegend({ u }: { u: Units }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
      <span className="cap" style={{ paddingBottom: 12 }}>
        TEMPERATURE {u.tu}
      </span>
      <svg
        width={W}
        height={23}
        style={{ display: 'block' }}
        aria-label={`Temperature scale ${u.t(lo)} to ${u.t(hi)}${u.tu}`}
      >
...
function DayTip({
  ...
  const left = Math.min(x + 14, window.innerWidth - 334),
    top = y + 16 + 230 > window.innerHeight ? y - 240 : y + 16
  ...
  return (
    <div className="floating" style={{ left, top }}>
      ...
      <div
        className="mono"
        style={{ display: 'grid', gridTemplateColumns: 'auto auto', gap: '2px 14px', color: 'var(--mid)' }}
      >
        <span>High / low</span>
        <span style={{ color: 'var(--ink)' }}>
          {u.t(inp.hi)} / {u.t(inp.lo)}
          {u.tu}
        </span>
        {mode === 'comfort' && (p.sun === 'sun' || p.basis === 'apparent') && (
          <>
            <span>Scored temp</span>
            <span style={{ color: 'var(--ink)' }}>
              {u.t(basisT)}
              {u.tu}
            </span>
        ...
        <span style={{ color: 'var(--ink)' }}>{Math.round(s.cloud[j])}%</span>
        <span>Precip / snow</span>
        <span style={{ color: 'var(--ink)' }}>
          {u.len(inp.precip)} / {u.len(s.snow[j], 1)}
        </span>
        <span>Max wind</span>
        <span style={{ color: 'var(--ink)' }}>{u.speed(inp.wind)}</span>
        <span>Sunshine</span>
        <span style={{ color: 'var(--ink)' }}>{Number.isNaN(s.sun[j]) ? '—' : `${s.sun[j].toFixed(1)} h`}</span>
        {row.terrain && (
          <>
            <span>Terrain snow</span>
            <span style={{ color: 'var(--ink)' }}>{Number.isNaN(inp.depth) ? '—' : u.depth(inp.depth)}</span>
          </>
        )}
      </div>
      {mode === 'comfort' && sc && (
        <div style={{ marginTop: 8, paddingTop: 7, borderTop: '1px solid #3a414c' }}>
          <span
            style={{ color: BAND_COLOR[sc.band[i]] === CO.unb ? '#98a0ad' : BAND_COLOR[sc.band[i]], fontWeight: 600 }}
          >
            {['Comfortable', 'Tolerable', 'Unbearable'][sc.band[i]]}
          </span>
          {sc.band[i] !== BAND.unb && <span className="mono"> · fit {Math.round(sc.score[i])}</span>}
          {dayCause(sc, i) > 0 && <span> · {causeLabel(dayCause(sc, i))}</span>}
        </div>
      )}
      {mode === 'activity' && (
        <div
        ...
        >
          {ACTIVITIES.filter((a) => p.acts.includes(a.id)).map((a) => {
            ...
            return [
              ...
              <span key={a.id + 'r'} className="mono" style={{ color: r ? 'var(--dim)' : CO.act }}>
                {r ? `✕ ${LOSS_LABEL[r]}` : '✓'}
              </span>,
            ]
          })}
