...
export function ControlBar(props: BarProps) {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (y < 60) { pinned.current = false; setCollapsed(false) }
      else if (!pinned.current) setCollapsed(true)
      else if (Math.abs(y - lastY.current) > 400) { pinned.current = false; setCollapsed(true); setMenu(null) }
  ...
  useEffect(() => {
    const k = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); pinned.current = true; setCollapsed(false); setMenu('city') }
    }
    ...
    <div className="bar" ref={root}>
      <div className="bar-row">
        ...
        <div className="bar-cell row" style={{ minWidth: 190 }}>
          <button className="city-btn" aria-expanded={menu === 'city'} onClick={() => toggle('city')} data-tip="Switch city. ⌘K opens the same picker from anywhere.">
            <span className="cap">CITY</span>
            <span style={{ font: '600 13px/1 var(--sans)', whiteSpace: 'nowrap' }}>{city.name}, {city.code}</span>
        ...
        <div className="bar-cell" style={{ width: 372 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
            <Cap tip="Your ideal temperature range (green handles) plus hard bounds (the thin floor and ceiling handles), outside which a day is written off. Between ideal and a hard bound a day degrades linearly — that is the tolerable ramp. Drag a hard handle off either end of the track to make that side open-ended. Scale −30 to 110°F.">
              COMFORT BAND · {u.tu}{p.seasonal ? ' · WARM SEASON' : ''}
            </Cap>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button className={`chip${t && t.hardMin === null ? ' open' : ''}`} disabled={!t}
              onClick={() => t && set({ temp: { ...t, hardMin: t.hardMin === null ? Math.max(T_MIN, t.idealMin - 15) : null } })}
            ...
            <span className="cap" style={{ flex: 1, textAlign: 'center' }}>{t ? 'DRAG · OFF-TRACK = OPEN' : 'NOT SCORED UNTIL SET'}</span>
            <button className={`chip${t && t.hardMax === null ? ' open' : ''}`} disabled={!t}
              onClick={() => t && set({ temp: { ...t, hardMax: t.hardMax === null ? Math.min(T_MAX, t.idealMax + 10) : null } })}
        ...
        <div className="bar-cell" style={{ width: 150 }}>
          ...
          <input type="range" min={30} max={78} step={1} value={p.dewMax ?? 78} className={p.dewMax === null ? 'unset' : ''}
            onChange={(e) => set({ dewMax: +e.target.value })} aria-label="Dew point ceiling" />
        </div>

        <div className="bar-cell">
          <Cap tip="Where the line between COMFORTABLE and TOLERABLE sits on the 0–100 fit score: Lenient 75, Standard 85, Strict 95. It moves the cutoff only — your hard bounds do not change.">STRICTNESS</Cap>
          <Seg label="Strictness" value={p.strict} onChange={(v) => set({ strict: v })}
            options={[{ v: 'lenient', label: 'Lenient' }, { v: 'standard', label: 'Standard' }, { v: 'strict', label: 'Strict' }]} />
        </div>

        <div className="bar-cell">
          <Cap tip="How far you will drive for snow. Snow metrics are measured at the best real terrain inside that limit, never at city centre where it may rarely snow.">SNOW DRIVE</Cap>
          <Seg label="Snow drive" value={p.drive} onChange={(v) => set({ drive: v })}
            options={[{ v: 1, label: '≤1h' }, { v: 2, label: '≤2h' }, { v: 3, label: '≤3h' }]} />
        </div>

        <div className="bar-cell">
          <Cap tip="How many years of daily weather to score. 3–5 years reflects the climate as it is now; 20–30 years is steadier but averages in an earlier climate. Every figure on this page is the mean per year over this window.">LOOKBACK · {windowLabel(p.window)}</Cap>
          <div style={{ display: 'flex', gap: 2 }}>
            <Seg label="Lookback years" value={isPreset ? n : -1} onChange={(v) => set({ window: { from: LAST_YEAR - v + 1, to: LAST_YEAR } })} options={LOOKBACKS} />
            <div className="seg">
              <button aria-pressed={!isPreset} onClick={() => toggle('window')} data-tip="Custom start–end years.">⋯</button>
            </div>
          </div>
          {menu === 'window' && (
            <div className="pop" style={{ right: 0, width: 230, padding: 12, gap: 10 }}>
              <span className="cap">CUSTOM WINDOW · {FIRST_YEAR}–{LAST_YEAR} AVAILABLE</span>
              <div className="builder" style={{ margin: 0 }}>
                <select value={p.window.from} onChange={(e) => set({ window: { from: +e.target.value, to: Math.max(+e.target.value, p.window.to) } })}>
                  {yearsList().map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
                <span>TO</span>
                <select value={p.window.to} onChange={(e) => set({ window: { from: Math.min(p.window.from, +e.target.value), to: +e.target.value } })}>
                  {yearsList().map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
            </div>
        ...
        <div className="bar-cell">
          <Cap tip="Which temperature drives the score. SHADE uses reported air temperature. IN SUN adds a solar-radiation term, so clear, high-irradiance days read warmer than the thermometer — 90°F and sunny in Denver is not 90°F and sunny in Atlanta.">BASIS</Cap>
          <Seg label="Sun or shade" value={p.sun} onChange={(v) => set({ sun: v })} options={[{ v: 'shade', label: 'Shade' }, { v: 'sun', label: 'In sun' }]} />
        </div>
      ...
      <div className="bar-row2">
        ...
        <div className="bar-end">
          ...
          <button className="btn" aria-expanded={menu === 'presets'} onClick={() => toggle('presets')} data-tip="Starting profiles, alphabetical. Each overwrites the band, dew point and sky preference; every value it sets is printed under its name.">Presets ▾</button>
          <button className="btn primary" aria-expanded={menu === 'more'} onClick={() => toggle('more')} data-tip="Seasonal bands, scoring basis, sky, wind, rain, weights and deal-breakers.">More controls ▾</button>

          {menu === 'presets' && (
            <div className="pop" style={{ right: 0, width: 340 }}>
              <span className="cap" style={{ padding: '6px 8px 4px' }}>STARTING POINTS · OVERWRITES THE BAND</span>
              {PRESETS.map((pr) => (
                <button key={pr.name} className="pop-item" onClick={() => { set(pr.apply); setMenu(null) }}>
          ...
          {menu === 'more' && (
            <div className="pop more">
              <div className="more-row">
                <div className="line">
                  <button className={`toggle${p.seasonal ? ' on' : ''}`} aria-pressed={p.seasonal} aria-label="Seasonal bands"
                    onClick={() => set({ seasonal: !p.seasonal, ...(t && !p.seasonal ? { cold: { idealMin: Math.max(T_MIN, t.idealMin - 10), idealMax: Math.max(T_MIN + 2, t.idealMax - 10) } } : {}) })}>
                    <span />
                  </button>
                  <span style={{ font: '400 11.5px/1.3 var(--sans)' }}>Seasonal bands — {p.seasonal ? 'on: separate cold-season band' : 'off: one band all year'}</span>
                </div>
                {p.seasonal && (
                  <>
                    <div className="line" style={{ justifyContent: 'space-between' }}>
                      <span className="cap">COLD-SEASON IDEAL · {u.tu}</span>
                      <span className="readout">{u.t(p.cold.idealMin)} – {u.t(p.cold.idealMax)}</span>
                    </div>
                    <FourPointSlider label="Cold-season band" hardEditable={false} fixedHard={t ? { hardMin: t.hardMin, hardMax: t.hardMax } : undefined}
                    ...
                    <span className="note">Temperature only; hard bounds are shared with the main band. {warmNote} The two bands blend across each season boundary.</span>
                  </>
                )}
              </div>

              <div className="more-row">
                <Cap tip="Which daily temperature the band is compared against. Overnight lows matter for sleep and are routinely ignored elsewhere. BOTH requires the high and the low to qualify.">SCORING BASIS</Cap>
                <Seg label="Scoring basis" value={p.basis} onChange={(v) => set({ basis: v })}
                  options={[{ v: 'high', label: 'Daily high' }, { v: 'low', label: 'Daily low' }, { v: 'apparent', label: 'Apparent' }, { v: 'both', label: 'High & low' }]} />
              </div>

              <VarRow label="SKY" tip="Cloud cover preference. Prefer overcast: ideal ≥ 60% daily mean cloud. Prefer clear: ideal ≤ 40%. Scores fade over 40 points beyond ideal.">
                <Seg small label="Sky" value={p.cloud} onChange={(v) => set({ cloud: v })}
                  options={[{ v: 'any', label: 'Any' }, { v: 'clear', label: 'Prefer clear' }, { v: 'overcast', label: 'Prefer overcast' }]} />
                {p.cloud !== 'any' && <Extras k="cloud" p={p} set={set} />}
              </VarRow>

              <VarRow label={`WIND CEILING${p.windMax !== null ? ` · ${u.speed(p.windMax)}` : ' · OFF'}`} tip="Daily maximum sustained wind (not gusts). Above the ceiling the score fades over 15 mph.">
                <input type="range" min={6} max={40} step={1} value={p.windMax ?? 40} className={p.windMax === null ? 'unset' : ''} style={{ width: 160 }}
                  onChange={(e) => set({ windMax: +e.target.value })} aria-label="Wind ceiling" />
                {p.windMax !== null && <><button className="x-btn" onClick={() => set({ windMax: null })}>✕</button><Extras k="wind" p={p} set={set} /></>}
              </VarRow>

              <VarRow label="PRECIPITATION" tip={'Prefer dry: ideal ≤ 0.02" in the day; the score fades to zero at 0.5".'}>
                <Seg small label="Precipitation" value={p.dry ? 'dry' : 'any'} onChange={(v) => set({ dry: v === 'dry' })}
                  options={[{ v: 'any', label: 'Any' }, { v: 'dry', label: 'Prefer dry' }]} />
                {p.dry && <Extras k="precip" p={p} set={set} />}
              </VarRow>

              <div className="more-row">
                <Cap tip="How much each variable counts in the 0–100 fit score. Weights normalise across whichever variables are active.">WEIGHTS · TEMPERATURE / DEW POINT</Cap>
                <div className="line">
                  <span className="cap" style={{ width: 34 }}>TEMP</span>
                  <Seg small value={p.weights.temp} onChange={(v) => set({ weights: { ...p.weights, temp: v } })} options={WEIGHTS} />
                </div>
                <div className="line">
                  <span className="cap" style={{ width: 34 }}>DEW</span>
                  <Seg small value={p.weights.dew} onChange={(v) => set({ weights: { ...p.weights, dew: v } })} options={WEIGHTS} />
                </div>
              </div>

              <div className="more-row">
                <Cap>CALENDAR ENCODING</Cap>
                <Seg small value={props.calFill} onChange={props.setCalFill} options={[{ v: 'banded', label: 'BANDED' }, { v: 'continuous', label: 'CONTINUOUS' }]} />
              </div>

              <div className="more-row" style={{ borderTop: '1px solid var(--line)', paddingTop: 12 }}>
                <div className="line">
                  <button className="link-btn" onClick={() => { props.reset(); setMenu(null) }}>CLEAR ALL PREFERENCES</button>
                  <button className="link-btn" onClick={() => { set(EXAMPLE_STATE); setMenu(null) }}>LOAD SPEC EXAMPLE STATE</button>
                </div>
                <span className="note">Cutoff now ≥ {CUTOFF[p.strict]} · window {windowLabel(p.window)} ({windowYears(p.window)} yr).</span>
              </div>
            </div>
          )}
        </div>
...
function Extras({ k, p, set }: { k: 'cloud' | 'wind' | 'precip'; p: Prefs; set: (patch: Partial<Prefs>) => void }) {
  return (
    <>
      <Seg small label="Weight" value={p.weights[k]} onChange={(v) => set({ weights: { ...p.weights, [k]: v } })} options={WEIGHTS} />
      <button className={`chip${p.deal[k] ? ' open' : ''}`} onClick={() => set({ deal: { ...p.deal, [k]: !p.deal[k] } })}
