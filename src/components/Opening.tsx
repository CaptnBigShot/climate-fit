  ...
  return (
    <div className="section">
      <div className="section-head">
        <Head tip="Ordered alphabetically. The cold and gray options carry the same visual weight as the warm ones — the app has no opinion about which of these is good weather.">
          STARTING POINTS
        </Head>
        <span className="sub">
          alphabetical · each overwrites the band · every threshold published in{' '}
          <button className="inline-link" onClick={openMethods}>
            data &amp; methods
          </button>
        </span>
      </div>
      <div className="preset-grid">
        ...
        <button
          className="preset-card example"
          onClick={() => set(EXAMPLE_STATE)}
...
export function OutdoorRanking({
  rows,
  current,
  w,
  openCity,
  ...
  return (
    <div className="section">
      <div className="section-head">
        <Head tip="Walk-viable days are a physical constraint with a published threshold, not a taste assumption — which is why this ranking is legitimate before any preference is stated.">
          WHERE COULD YOU BE OUTSIDE MOST DAYS
        </Head>
      ...
      <div className="rank-grid" style={{ gridTemplateRows: `repeat(${Math.ceil(CITIES.length / 2)}, auto)` }}>
        {ranked.map((r, i) => (
          <button
            key={r.city.id}
            className="rank-row"
            aria-current={r.city.id === current ? 'true' : undefined}
            onClick={() => openCity(r.city.id)}
        ...
        {pending > 0 && (
          <span className="cap" style={{ padding: '8px 0' }}>
            loading {pending} more {pending === 1 ? 'city' : 'cities'}…
          </span>
        )}
      </div>
    </div>
