  ...
  return (
    <div className="section" style={{ borderBottom: 'none' }}>
      <div className="section-head">
        <Head tip="For travel rather than relocation: the date ranges where your comfortable days have clustered, across every year in the window — and the stretch to avoid.">
          BEST TIME TO VISIT
        </Head>
        <div className="right">
          <Seg
      ...
      {!r ? (
        <NeedsPreference what="rank date ranges" />
      ) : (
        <>
          <div className="sub" style={{ marginBottom: 8 }}>
            share of comfortable days in each span · {windowLabel(p.window)}
          </div>
          {r.best.map((sp, i) => (
            <SpanRow key={sp.start} rank={`${i + 1}`} sp={sp} />
          ))}
          <div style={{ borderTop: '1px solid var(--line)', marginTop: 14, paddingTop: 12 }}>
            <div style={{ marginBottom: 8 }}>
              <Head
                small
                tip="The span with the most unbearable days — the stretch of the year you would most want to be elsewhere."
              >
                WORST {len === 30 ? '30 DAYS' : len === 7 ? 'WEEK' : 'TWO WEEKS'} OF THE YEAR
              </Head>
            </div>
...
function SpanRow({ rank, sp, worst }: { rank: string; sp: DateSpan; worst?: boolean }) {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid var(--row)' }}
    >
      <span className="mono" style={{ width: 14, color: worst ? CO.warn : 'var(--faint)', fontSize: 11 }}>
        {rank}
      </span>
      <span className="mono" style={{ width: 130, fontSize: 12, color: 'var(--ink)' }}>
        {spanLabel(sp)}
      </span>
      <span
        style={{ display: 'flex', height: 8, flex: 1, background: 'var(--row)' }}
        data-tip={`Comfortable ${pct(sp.comf)} · tolerable ${pct(tol)} · unbearable ${pct(sp.unb)} of days in this span · mean fit ${Math.round(sp.score)}`}
      >
        <span style={{ width: pct(sp.comf), background: CO.comf }} />
        <span style={{ width: pct(tol), background: CO.tol }} />
        <span style={{ width: pct(sp.unb), background: CO.unb }} />
      </span>
      <span
        className="mono"
        style={{ width: 44, textAlign: 'right', fontSize: 11, color: worst ? 'var(--unb-text)' : CO.comf }}
      >
        {pct(worst ? sp.unb : sp.comf)}
      </span>
    </div>
  )
}
...
export function TypicalDayPanel({
  city,
  p,
  u,
  sc,
  ...
  if (prof && prof.years && W) {
    ...
    let lo = Math.min(...prof.p10),
      hi = Math.max(...prof.p90)
    if (band) {
      lo = Math.min(lo, band[0] - 2)
      hi = Math.max(hi, band[1] + 2)
    }
    ...
    chart = (
      <svg width={W} height={H} style={{ display: 'block' }}>
        {sun && (
          <rect
            x={x(Math.max(0, sun.rise))}
            width={x(Math.min(24, sun.set)) - x(Math.max(0, sun.rise))}
            y={T}
            height={H - B - T}
            fill="#e6e8ec"
            fillOpacity={0.04}
        ...
        {ticks.map((v) => (
          <g key={v}>
            <line x1={L} x2={W - R} y1={y(v)} y2={y(v)} stroke={CO.grid} />
            <text
              x={L - 5}
              y={y(v) + 3}
              textAnchor="end"
  ...
  return (
    <div className="section" style={{ borderBottom: 'none' }} ref={ref}>
      <div className="section-head">
        <Head tip="Hour-by-hour temperature for a typical day in the chosen month, local clock time. Bands are the 10th–90th and 25th–75th percentile across every day of that month; the line is the median. Shaded: sunrise to sunset. Exposes the day–night swing that daily figures flatten.">
          TYPICAL DAY
        </Head>
        <div className="right">
          <select
      ...
      <div className="sub" style={{ marginBottom: 8 }}>
        hourly tier ·{' '}
        {prof?.years
          ? `${prof.years[0]}–${prof.years[1]} · ${prof.days} days`
          : hs
            ? `${hs.startYear}–${hs.startYear + hs.years - 1} only — outside this window`
            : err
              ? 'unavailable'
              : 'loading (a city added recently is fetched live the first time)'}{' '}
        · {hs?.timezone ?? ''}
        {hs?.live ? ' · fetched live from Open-Meteo, cached in this browser' : ''}
      </div>
      ...
      {prof?.years && sun && (
        <div style={{ display: 'flex', gap: 22, marginTop: 10 }}>
          {[
            ['DAWN', at(sun.rise)],
            ['AFTERNOON PEAK', prof.p50[warmest]],
            ['DUSK', at(sun.set)],
            ['MEDIAN SWING', Math.max(...prof.p50) - Math.min(...prof.p50)],
          ].map(([k, v], i) => (
            <div key={k as string}>
              <div className="cap">{k}</div>
              <div className="mono" style={{ fontSize: 14, marginTop: 3 }}>
                {i === 3 ? u.dt(v as number).replace('+', '') : u.t(v as number)}
                {u.tu}
              </div>
            </div>
          ))}
    ...
    return (
      <div className="section" style={{ borderBottom: 'none' }} ref={ref}>
        <div style={{ marginBottom: 12 }}>
          <Head>WHAT WOULD HAVE TO CHANGE</Head>
        </div>
        <NeedsPreference what="see how warming would move your day budget" />
      </div>
    )
  }
    ...
    <div className="section" style={{ borderBottom: 'none' }} ref={ref}>
      ...
      <div className="prose" style={{ marginBottom: 10 }}>
        {now.comf < target ? (
          ...
          <>
            Even {u.dt(SHIFTS[SHIFTS.length - 1], 0)}
            {u.tu} of warming keeps this city above {target} comfortable days for you.
          </>
        ) : (
          <>
            Comfortable days fall below <b className="mono">{target}</b> after about{' '}
            <b className="mono">
              {u.dt(cross)}
              {u.tu}
            </b>{' '}
            ...
            {yr ? (
              <>
                At the {trend.from}–{trend.to} trend of{' '}
                <b className="mono">
                  {u.dt(perDecade, 2)}
                  {u.tu}/decade
                </b>{' '}
                (R² {trend.r2.toFixed(2)}), that is around <b className="mono">{yr}</b>.
              </>
            ) : (
              <>
                This city shows no warming trend over {trend.from}–{trend.to}.
              </>
            )}
          </>
        )}
      ...
      {W > 0 && (
        <svg width={W} height={H + 18} style={{ display: 'block' }}>
          ...
          />
          {sens.map((x, i) => {
            const cH = (x.comf / max) * H,
              uH = (x.unb / max) * H
            return (
              <g
                key={x.shift}
                data-tip={`${u.dt(x.shift, 0)}${u.tu}: ${Math.round(x.comf)} comfortable, ${Math.round(x.unb)} unbearable days/yr`}
              >
                <rect
                  x={10 + i * bw + 2}
                  y={H - cH}
                ...
                <text
                  ...
                  y={H + 13}
                  textAnchor="middle"
                  fill={x.shift === 0 ? 'var(--ink)' : CO.faint}
                  style={{ font: "400 9px 'JetBrains Mono', monospace" }}
                >
                  {x.shift === 0 ? 'NOW' : u.dt(x.shift, 0)}
                </text>
              </g>
            )
          })}
      ...
      <div className="legend" style={{ marginTop: 6 }}>
        <span className="item">
          <span className="sw" style={{ background: CO.comf, width: 12 }} />
          COMFORTABLE
        </span>
        <span className="item">
          <span className="sw" style={{ background: CO.unb, width: 12 }} />
          UNBEARABLE
        </span>
        <span className="item">
          <span className="sw" style={{ background: CO.warn, height: 1, width: 14 }} />
          {target}-DAY LINE
        </span>
      </div>
    </div>
...
export function ExtremesPanel({ s, p, u }: { s: CitySeries; p: Prefs; u: Units }) {
  ...
  const fmt = (e: (typeof ex)[number]) =>
    e.kind === 'temp'
      ? `${u.t(e.value)}${u.tu}`
      : e.kind === 'delta'
        ? `${u.dt(e.value, 0).replace('+', '')}${u.tu}`
        : e.kind === 'len'
          ? u.len(e.value, u.metric ? 0 : 2)
          : u.speed(e.value)
  const date = (i: number) => `${doyLabel(i % 365)}, ${p.window.from + Math.floor(i / 365)}`
  return (
    <div className="section" style={{ borderBottom: 'none' }}>
      <div style={{ marginBottom: 12 }}>
        <Head tip="The single most extreme day of each kind inside the window, with its date, so the records are concrete events rather than statistics.">
          ANNOTATED EXTREMES
        </Head>
      </div>
      <div className="sub" style={{ marginBottom: 8 }}>
        single days · {windowLabel(p.window)}
      </div>
      {ex.map((e) => (
        <div className="list-row" key={e.label}>
          ...
          <span className="v" style={{ width: 78 }}>
            {fmt(e)}
          </span>
        </div>
      ))}
...
export function AirQualityPanel({ city, w }: { city: CityMeta; w: Window }) {
  ...
  return (
    <div className="section" style={{ borderBottom: 'none' }}>
      <div style={{ marginBottom: 12 }}>
        <Head
          tip={`Days per year when the US Air Quality Index went above each EPA category edge, over your lookback window clipped to the air-quality record. A day's AQI is its worst pollutant; each pollutant row counts that pollutant's own index, so one day can count under more than one. US cities use EPA monitors within ${EPA_RADIUS_KM} km; elsewhere, the CAMS model.`}
        >
          AIR QUALITY
        </Head>
      </div>
      ...
      {cur && !st && (
        <div className="prose">
          The air-quality record covers {cur.start} → {cur.end}, outside the {windowLabel(w)} window.
        </div>
      )}
      {cur && st && (
        <>
          ...
          {st.worst && (
            <div className="list-row" data-tip={`The highest AQI in the span, set by ${pollutantLabel(st.worst.by)}.`}>
              <span className="k">Worst day</span>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--dim)' }}>
                {st.worst.date} · {pollutantLabel(st.worst.by)}
              </span>
              <span className="v">AQI {st.worst.aqi}</span>
            </div>
          )}
          <AqMonths st={st} />
    ...
    <table className="aq" style={{ marginBottom: 4 }}>
      <thead>
        <tr>
          ...
          {AQI_LEVELS.map((l) => (
            <th key={l.at} data-tip={`AQI above ${l.at}: ${l.name}.`}>
              AQI {l.at}
            </th>
          ))}
        </tr>
            ...
            <td className={`l${row.key === 'any' ? ' strong' : ''}`} style={{ fontFamily: 'var(--sans)' }}>
              <span
                style={{
                  ...
                  height: 8,
                  marginRight: 7,
                  background: swatch(row.key) ?? 'transparent',
                }}
              />
              {row.label}
            </td>
            {row.perYear.map((v, k) => (
              <td key={k} className={row.key === 'any' ? 'strong' : undefined}>
                {fmtDays(v)}
              </td>
            ))}
          </tr>
...
function AqMonths({ st }: { st: AqStats }) {
  ...
  return (
    <>
      <div className="cap" style={{ margin: '14px 0 6px' }}>
        DAYS AQI &gt; {AQI_TRACKED} BY MONTH · PER YEAR
      </div>
      ...
      <div style={{ display: 'flex' }}>
        {MN.map((n) => (
          <span key={n} className="cap" style={{ flex: 1, textAlign: 'center', fontSize: 8.5 }}>
            {n[0]}
          </span>
        ))}
      </div>
      {shown.length > 0 && (
        <div className="legend" style={{ marginTop: 6 }}>
          {shown.map((l) => (
            <span key={l.key} className="item">
              <span className="sw" style={{ background: AQ_COLOR[l.key], width: 12 }} />
              {l.label}
            </span>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 14 }}>
      <span className="cap" style={{ flex: 1 }}>
        AQI &gt; {AQI_TRACKED} PER YEAR · {st.byYear[0].year}–{st.byYear[st.byYear.length - 1].year}
      </span>
      <span data-tip={st.byYear.map((y) => `${y.year}: ${y.days}`).join(' · ')}>
        <Spark ys={ys} w={96} h={24} color={AQ_COLOR.o3} />
      </span>
      <span className="mono" style={{ fontSize: 10, color: 'var(--dim)', whiteSpace: 'nowrap' }}>
        {signed(fit.slope, 1)} /yr · R² {fit.r2.toFixed(2)}
      </span>
    </div>
  )
}
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      {POLLUTANTS.map((p) => {
        ...
        const list = sites
          .slice(0, 5)
          .map((s) => `${s.name} (${s.km} km): ${s.days.toLocaleString()} days`)
          .join(' · ')
        ...
        return (
          <div key={p.key} className="list-row" data-tip={`${rule} ${list}${more}`}>
            ...
            <span className="mono" style={{ fontSize: 10.5, color: 'var(--dim)', textAlign: 'right' }}>
              {highest
                ? `highest within ${EPA_RADIUS_KM} km · ${sites.length} monitors`
                : `${sites[0].name} · ${sites[0].km} km${sites.length > 1 ? ` +${sites.length - 1}` : ''}`}
            </span>
          </div>
        )
      })}
...
export function MosquitoPanel({ s, p, u }: { s: CitySeries; p: Prefs; u: Units }) {
  ...
  return (
    <div className="section" style={{ borderBottom: 'none' }}>
      <div style={{ marginBottom: 12 }}>
        <Head
          tip={`A rough proxy, not an observation: days whose overnight low stays at or above ${MOSQUITO_LOW_F}°F and whose mean dew point is at or above ${MOSQUITO_DEW_F}°F — warm and humid enough for mosquitoes to be active through the day. Says nothing about species, standing water or local control programmes.`}
        >
          MOSQUITO-FAVOURABLE DAYS
        </Head>
      </div>
      <div className="sub" style={{ marginBottom: 10, color: 'var(--warn)' }}>
        rough proxy — not an observation · low ≥ {u.t(MOSQUITO_LOW_F)}
        {u.tu} and dew pt ≥ {u.t(MOSQUITO_DEW_F)}
        {u.tu}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div className="stat">
          <div className="n" style={{ color: 'var(--ink)', fontSize: 26 }}>
            {Math.round(mq.avg)}
          </div>
          <div className="d">days/yr · {windowLabel(p.window)}</div>
        </div>
        <Spark ys={mq.perYear} w={120} h={30} color="#8a9ab3" />
        <span className="mono" style={{ fontSize: 10, color: 'var(--dim)' }}>
          {signed(mq.fit.slope, 2)} /yr · R² {mq.fit.r2.toFixed(2)}
        </span>
      </div>
      <div className="cap" style={{ margin: '14px 0 6px' }}>
        BY MONTH · DAYS PER YEAR{mq.avg >= 1 ? ` · PEAK ${MN[peak].toUpperCase()}` : ''}
      </div>
      <svg width="100%" height={46} viewBox="0 0 240 46" preserveAspectRatio="none" style={{ display: 'block' }}>
        {mq.months.map((v, i) => (
          <rect
            ...
            height={(v / maxM) * 34}
            fill="#8a9ab3"
            data-tip={`${MN[i]}: ${v.toFixed(1)} days/yr`}
          />
        ))}
      </svg>
      <div style={{ display: 'flex' }}>
