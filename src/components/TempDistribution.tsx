...
export function TempDistribution({ s, p, u, ytd }: { s: CitySeries; p: Prefs; u: Units; ytd: Ytd | null }) {
  ...
  const stats = useMemo(() => {
    const q = {
      p10: new Float32Array(365),
      p25: new Float32Array(365),
      p50: new Float32Array(365),
      p75: new Float32Array(365),
      p90: new Float32Array(365),
      min: new Float32Array(365),
      max: new Float32Array(365),
    }
    const col = new Float32Array(years)
    for (let d = 0; d < 365; d++) {
      for (let y = 0; y < years; y++) col[y] = src[off + y * 365 + d]
      col.sort()
      q.p10[d] = quantileSorted(col, 0.1)
      q.p25[d] = quantileSorted(col, 0.25)
      q.p50[d] = quantileSorted(col, 0.5)
      q.p75[d] = quantileSorted(col, 0.75)
      q.p90[d] = quantileSorted(col, 0.9)
      q.min[d] = col[0]
      q.max[d] = col[years - 1]
    }
    return q
  ...
  let lo = Math.min(...stats.min),
    hi = Math.max(...stats.max)
  if (t) {
    lo = Math.min(lo, t.idealMin - 4, p.seasonal ? p.cold.idealMin - 4 : Infinity)
    hi = Math.max(hi, t.idealMax + 4, t.hardMax !== null ? t.hardMax + 4 : -Infinity)
  }
  ...
  const stepPts = (a: ArrayLike<number>) => {
    const pts: string[] = []
    for (let d = 0; d < 365; d++)
      pts.push(`${x(d).toFixed(1)},${yv(a[d]).toFixed(1)}`, `${x(d + 1).toFixed(1)},${yv(a[d]).toFixed(1)}`)
    return pts
  }
  ...
  for (let i = 1; i < years * 365; i++)
    if (which === 'high' ? src[off + i] > src[off + ri] : src[off + i] < src[off + ri]) ri = i
  const rv = src[off + ri],
    rd = ri % 365,
    rx = x(rd + 0.5),
    ry = yv(rv)
  ...
  const recMark = (
    <g>
      ...
      <text
        x={rx + (rd > 250 ? -7 : 7)}
        y={ry + (which === 'high' ? -6 : 13)}
        textAnchor={rd > 250 ? 'end' : 'start'}
        fill="#ffd18a"
        style={{ font: "500 9.5px 'JetBrains Mono', monospace" }}
      >
        {recLabel}
      </text>
    </g>
  ...
  if (mode === 'ytd' && ytd && ytd.raw.days > 0) {
    const n = ytd.raw.days,
      y0 = (YTD_YEAR - FIRST_YEAR) * 365,
      ys = which === 'high' ? ytd.sx.high : ytd.sx.low
    const pts: string[] = []
    let above = 0,
      below = 0
    for (let d = 0; d < n; d++) {
    ...
    ytdLine = (
      <g>
        ...
        <text x={x(n) + 5} y={PAD_T + 10} fill="#8b929e" style={{ font: "400 9px 'JetBrains Mono', monospace" }}>
          {doyLabel(n - 1).toUpperCase()} · LAST OBSERVED
        </text>
      </g>
    )
    const expect = n * 0.1
    ytdNote = (
      <>
        {YTD_YEAR} so far: <b>{above}</b> days above the window's 90th percentile and <b>{below}</b> below its 10th,
        where a typical year would have about <b>{Math.round(expect)}</b> of each by {doyLabel(n - 1)}.
      </>
    )
  }
  const yearLine =
    mode === 'year' ? (
      <polyline
        points={stepPts(src.subarray(off + (year - p.window.from) * 365, off + (year - p.window.from + 1) * 365)).join(
          ' ',
        )}
        fill="none"
        stroke="#ffd18a"
  ...
  return (
    <div ref={ref}>
      <div className="section-head">
        <Head tip="Daily temperature through the year. Dark envelope: record range. Inner bands: 10th–90th and 25th–75th percentiles. Pale line: median. Green shading: your ideal band (stepping between seasonal bands when seasonal mode is on). Air temperature as observed, before any in-sun adjustment.">
          TEMPERATURE DISTRIBUTION
        </Head>
        <span className="sub">
          daily {which} · p10/p25/p50/p75/p90 + record envelope · step segments, no smoothing
          {t ? '' : ' · no comfort overlay until a band exists'}
        </span>
        <div className="right">
          <Seg
            small
          ...
          <Seg
            ...
            onChange={setMode}
            options={[
              { v: 'dist', label: 'DISTRIBUTION' },
              ...(ytd ? [{ v: 'ytd' as Mode, label: `${YTD_YEAR} SO FAR` }] : []),
              { v: 'every', label: 'EVERY DAY' },
              { v: 'year', label: 'SINGLE YEAR' },
            ]}
          />
          {mode === 'year' && (
            <select
            ...
            >
              {Array.from({ length: years }, (_, i) => p.window.from + i).map((y) => (
      ...
      {ytdNote && (
        <div className="legend">
          <span className="item">
            <span className="sw" style={{ background: '#f4f6f9', height: 2 }} />
            {YTD_YEAR} DAILY {which.toUpperCase()} · {ytd?.source === 'live' ? 'LIVE' : 'SNAPSHOT'}
          </span>
          <span className="prose" style={{ fontSize: 11 }}>
            {ytdNote}
          </span>
        </div>
      )}
      {mode === 'year' && (
        <div className="legend">
          <span className="item">
            <span className="sw" style={{ background: '#ffd18a', height: 2 }} />
            {year} DAILY {which.toUpperCase()} OVER THE {p.window.from}–{p.window.to} DISTRIBUTION
          </span>
        </div>
      )}
    </div>
