...
import { AirQualityPanel, BestTimePanel, ExtremesPanel, MosquitoPanel, TypicalDayPanel, WhatWouldChangePanel } from './Extras'
import { OutdoorRanking, StartingPoints } from './Opening'

export function CityView({ city, s, m, p, u, set, ytd, ytdSc, ytdError, manifest, calFill, setCalFill, cmp, toggleCompare, rows, openCity, openMethods }: {
  city: CityMeta; s: CitySeries; m: Model; p: Prefs; u: Units; set: (patch: Partial<Prefs>) => void
  ytd: Ytd | null; ytdSc: Scored | null; ytdError: string | null; manifest: Manifest | null
  calFill: CalFill; setCalFill: (v: CalFill) => void
  cmp: string[]; toggleCompare: () => void; rows: CityRow[]; openCity: (id: string) => void; openMethods: () => void
}) {
  ...
  return (
    <main className="page">
      ...
      <Header city={city} m={m} p={p} u={u} elevFt={s.demElevM * 3.28084} solarIdx={mf?.solarIdx ?? null}
        inCompare={cmp.includes(city.id)} compareCount={cmp.length} toggleCompare={toggleCompare}
        exportCsv={() => downloadMonthlyCsv(city, m.months, p, u)} />
      {!unset && <Hero m={m} p={p} ytd={ytd} ytdSc={ytdSc} ytdError={ytdError} />}
      <ComfortCalendar s={s} sc={m.sc} p={p} terrain={m.terrain?.series ?? null} u={u} fill={calFill} setFill={setCalFill}
        ytd={ytd} ytdSc={ytdSc} terrainId={m.terrain?.id ?? null} cityName={city.name.toUpperCase()} />
